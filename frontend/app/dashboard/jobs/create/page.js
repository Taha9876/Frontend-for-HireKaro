...
export default function CreateJobPage() {
    ...
    return (
        <div ref={pageRef} className="min-h-screen bg-slate-50 p-6 lg:p-10 relative overflow-hidden">
            {/* Background Orbs */}
            <div className="dash-orb" style={{ position: 'absolute', top: '5%', right: '15%', width: '35vw', height: '35vw', background: 'radial-gradient(circle, rgba(139,92,246,0.15) 0%, rgba(255,255,255,0) 70%)', borderRadius: '50%', zIndex: 0, filter: 'blur(60px)' }} />
            <div className="dash-orb" style={{ position: 'absolute', bottom: '-15%', left: '-5%', width: '45vw', height: '45vw', background: 'radial-gradient(circle, rgba(59,130,246,0.1) 0%, rgba(255,255,255,0) 70%)', borderRadius: '50%', zIndex: 0, filter: 'blur(80px)' }} />
            ...
            <div className="page-header mb-8">
                <button onClick={() => router.push('/dashboard/jobs')}
                    className="flex items-center gap-2 text-sm text-slate-400 hover:text-blue-600 transition-colors bg-transparent border-none cursor-pointer mb-4">
                    ← Back to Jobs
                </button>
            ...
            {/* Stepper */}
            <div className="stepper flex items-center gap-0 mb-10 bg-white/70 backdrop-blur-xl p-4 rounded-3xl border border-white/80 shadow-[0_8px_32px_rgba(59,130,246,0.05)]">
                {steps.map((step, idx) => (
                ...
                ${currentStep === step.number ? 'text-blue-600' : currentStep > step.number ? 'text-emerald-500' : 'text-slate-400'}`}>
                                {step.label}
                            </span>
                        </div>
                        {idx < steps.length - 1 && (
